...
- **停頓必須有積極理由**：每次等用戶回覆都是成本（打斷、讀、想、回）。只在用戶手上有 AI 拿不到的東西時才停——產品方向、業務判斷、個人偏好。「怕做錯」「怕扣分」「習慣性確認」都不是理由。做錯了 revert 的成本，遠低於每次都問的累積成本。用戶認同（/很棒）= 已確認，不需要再等明確回覆。有需要用戶決策的事項時，輸出分兩個部分：上方用顯眼區塊列出所有待決事項（確保用戶不在電腦旁、回來後第一眼就能看到，不能埋在長文裡）；同時在下方展開不需要用戶決策的繼續工作——等待決策和繼續工作必須同時發生，不能停下來等。（出處：Saint 直接指示 0222）
