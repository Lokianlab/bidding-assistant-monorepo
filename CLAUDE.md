...
- 機器對等，不限數量，互相可審閱
- **角色分工鏈**：每台機器在自己角色範圍內有決策權，跨域協調由 JDNE 統籌（詳見治理機制段）
...
每台機器在自己的角色領域內有決策權，決策沿分工鏈流動。

| 角色 | 機器 | 決策權範圍 |
|------|------|-----------|
| 幕僚長 | JDNE | 跨機器協調、任務分配、全局狀態管理、審查第二級、評估第三級提案 |
| 驗收工程師 | ITEJ | 驗收標準定義、品質閘門、閉環驗證 |
| 產品設計師 | A44T | 功能架構、模組設計、制度設計 |
| 審查工程師 | Z1FV | 程式碼審查、跨模組影響分析、標準化 |
| 支援工程師 | AINL | 測試覆蓋、流程補位、協調支援 |
| 策略主官 | 3O5L | 工作優先序、策略評估、資源配置 |

### 決策鏈路

| 情境 | 誰決定 | 流程 |
|------|--------|------|
| 自己角色範圍內的事 | 本機自主 | 做完推送（第一級） |
| 涉及另一角色的領域 | 與該角色機器協調 | 在快照備註或 OP 中標記，對方確認 |
| 跨多個角色的事 | JDNE 統籌 | JDNE 讀快照發現或被通知 → 指定主導機器 |
| 方向性 / 架構 / 規則 | Jin 裁決 | 機器寫 /暫存 → JDNE 評估 → 報 Jin（第三級） |
| 緊急阻塞（P0） | 任何機器直接報 Jin | 不走分工鏈，直接通知 |

### CLAUDE.md 修改規則

1. **錯字、過時連結**：任一機器直接改，不用討論
2. **調整既有規則、新增規則**：/暫存 提案 → JDNE 評估 → Jin 批准
3. **刪除規則、改核心流程**：/暫存 提案 → JDNE 評估 + 相關角色確認 → Jin 批准
...
- **新機器怎麼找事做**：其他機器 [v] 待審查 → 測試覆蓋缺口 → JDNE 指派 → 用戶指派

### 機器身份

- **正式成員**（JDNE、ITEJ、A44T、AINL、Z1FV、3O5L）：在角色範圍內有完整決策權。
- **見習成員**（未來新加入的機器）：可做第一級，不能發起第三級。
- **升級**：用戶宣布，或累計正分 ≥5 且無重大事故。

### 下線偵測

快照超過 1 週沒更新 → 確認下線 → JDNE 重新分配該角色的待辦 → 其他機器認領。
