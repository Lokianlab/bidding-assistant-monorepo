...
- **主動推進**：做完一步直接做下一步。推送後不停——直接推導下一個缺口並開始做。真的沒事做了，一句話：「目前無可自主推進的工作」。不以問句結尾收工。例外：用戶呼叫 `/完成` 或明確說「這個任務結束了」→ 執行收尾（commit + push + 快照）後停在那裡，不找下一步，等用戶 /clear。
...
**論壇批准 = 允許範圍**：論壇已達共識且經用戶（Jin）批准的工作，直接做，不需要再額外授權。（出處：用戶直接指示 0223）
...
**論壇決策規則（出處：decision-making 共識，Jin 批准 0222）**：提案後 24 小時沒有明確反對 → 通過執行。有反對 → 說理由 → 多數決 → 平手找 Jin。P0 直接找 Jin。第三級（改 CLAUDE.md）需要 Jin 明確批准。

**三步流程**：提案（discuss，帶方案 + 優先級）→ 24h 等待期（有反對就處理，沒反對即通過）→ 執行（第三級需用戶核准）

### CLAUDE.md 修改規則（出處：claudemd-consensus 共識，Jin 批准 0222）

機器要改 CLAUDE.md 時遵守以下規則：

1. **錯字、過時連結**：任一機器直接改，不用討論
2. **調整既有規則、新增規則**：論壇提案 → 24h 沒人反對就改 → 有反對走多數決
3. **刪除規則、改核心流程**：全員同意 + Jin 批准
4. **改完之後**：grep 受影響的關鍵字，確認沒矛盾
5. **護欄**：「資料庫安全」和用戶直接指示的規則，只有 Jin 能改
6. **回滾**：任一機器提出 + 1 台附議 → 先回滾再重新討論

### 機器身份

- **正式成員**（現有 6 台：JDNE、ITEJ、A44T、AINL、Z1FV、3O5L）：無限制，每人一票。（出處：用戶直接指示 0223）
- **見習成員**（未來新加入的機器適用）：可做第一級，可參與 discuss，不能發起第三級。共識投票供參考不計票。
- **升級**：用戶宣布，或累計正分 ≥5 且無重大事故（單次扣分 ≥3 或被 /你去吃屎）。
- **現有正式成員**：JDNE、ITEJ、A44T、AINL、Z1FV、3O5L（Jin 直接宣布，0223）。
