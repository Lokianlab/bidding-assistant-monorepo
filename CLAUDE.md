...
每當一個討論主題結束、或用戶切換話題時，Claude Code 必須自問：

1. **這次有沒有重要的決策或結論？** → 有的話用 `/暫存` 存進 `_staging/`
2. **有沒有影響所有機器的新知識？**（如技術棧變更、架構決策、新規範）→ 有的話更新 `CLAUDE.md`
3. **操作日誌更新了嗎？** → 重要操作必須記在 `docs/operation-log.md`
4. **以上都推上 GitHub 了嗎？** → 確認 `git push` 完成

**不能只寫進本機 MEMORY.md 就結束。** 其他機器看不到 MEMORY.md。
