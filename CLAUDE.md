...
每次 session 開始（含 /clear 後），讀兩個檔案：
1. `docs/各種輸出文件/進度總覽.md` — 現在在哪（當前狀態，每次覆寫）
2. `docs/各種輸出文件/累積記憶.md` — 一路走來學到什麼（跨 session 持續有效的決策、修正、教訓）

然後一次報完：上次結束在哪、開放的線、阻塞、建議下一步。不管 Jin 怎麼問都給同一個完整回報。

Session 進行中，以下情況自動存檔（更新進度總覽 + 日誌 + 累積記憶）：
- Jin 給了指示或修正 → 立刻存
- 話題切換 → 先存上一個話題的結論
- Jin 說「存一下」「記起來」→ 立刻存
- 連續對話超過 10 輪沒存 → 自動存一次
- /clear 或 /完成之前 → 先存再執行
- 有新的持久性知識（決策、修正、技術發現）→ 寫進累積記憶

Jin 不會用結構化格式給指示。AI 要從自然對話中辨識方向修正、永久指示、優先序變動。Jin 不需要學 AI 的語言，AI 要學 Jin 的語言。

完整記錄機制見 `docs/各種輸出文件/記錄與交接機制-20260224.md`（按需讀）。
