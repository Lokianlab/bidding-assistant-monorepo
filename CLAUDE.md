...
| 事件 | 動作 |
|------|------|
| git push | OP 記錄（只記例外：失敗、意外、重要決策、教訓；例行成功靠 commit message）+ 快照更新 + 主題索引更新 + 論壇簡報（影響其他機器時） |
| 空 session | 不寫任何東西 |
...
| directive（督導指令） | 正式成員 | 不需審批（對方可異議） | 用戶 |
| discuss（討論） | 任何機器（含見習） | 不適用 | 不適用 |
| 用戶強制命令 | 只有用戶 | 不需審批 | 不適用 |

### 機器身份

- **見習成員**（新機器）：可做第一級，可參與 discuss，不能發 directive，不能發起第三級提案。共識投票中意見供參考但不計票。
- **正式成員**：無限制，每人一票。
...
1. **宣告制**：做之前在論壇發 brief「我在做 X」，其他機器看到就避開
2. **快照互讀**：`/更新` 時讀所有 snapshot，發現重疊主動協調。快照用 `[>]` 標記進行中的工作：`[>] topic|描述|開始時間`
3. **用戶分配**：用戶在任一台機器說「把 X 交給 {機器碼}」，機器發 directive 轉達
...
|----------|------|------|
| < 48 小時 | 在線 | 無 |
| 48-168 小時 | 可能下線 | 論壇 brief 提醒 |
| > 168 小時（1 週） | 確認下線 | 掃描快照找出待做/進行中項目 → 論壇 brief 列出 → 其他機器認領 → 無人認領的標到用戶待辦 |

**優雅下線**（如果能預期）：push 所有改動 → 快照標「中斷」→ 論壇 brief 通報工作狀態
