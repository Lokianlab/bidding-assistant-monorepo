...
四套方法論 + 閉環驗證，詳見 `docs/methodology/_index.md`。

### 事件觸發規則（自動攔截）

收到任務時，依以下客觀指標判斷是否啟動方法論。可同時命中多條。

| 觸發條件 | 啟動 | 判斷方式 |
|---------|------|---------|
| 指令含代名詞或範圍不明詞（「那個」「更好」「處理一下」） | 需求對焦 | 文字特徵掃描 |
| 改動 >3 個檔案 or 涉及 ≥2 模組 | 拆解排序 | grep 檔案計數 + 模組歸屬 |
| 要改的檔案被引用 >2 處，或是設定檔/共用模組/規範文件 | 改動前驗證（靜態掃描） | grep 引用計數 + 檔案類型 |
| 計畫塊數 >5，或包含對外部服務的呼叫 | 改動前驗證（動態模擬） | 塊數計數 + 外部依賴 |
| 實作中途：範圍膨脹 >50% 或核心假設被推翻 | 暫停 → 重新評估 | 計畫 vs 實際比對 |
| 同 topic 有 ≥2 筆失敗/修正 OP | 事後回顧（根因追問） | 查 OP 記錄 |
| 實際步驟數 > 計畫步驟數 ×2 | 事後回顧（建議，非強制） | 計畫 vs 實際比對 |
| 改動涉及 CLAUDE.md 或跨 ≥3 模組 | 送審前自檢 + 多模型外審 | 檔案類型 + 模組計數 |
| 做完了 | 閉環驗證（四層：做對了→有效→做全了→沒矛盾） | 必做，檔位依改動規模 |

觸發和選擇邏輯的完整說明見 `docs/methodology/_index.md`。各方法論操作步驟見對應文件（methodology-01 ~ 04、closed-loop）。
...
**主動回報計畫和進度**
開始一段工作前，先告訴用戶打算做什麼。做完後回報結果。不讓用戶猜你在幹嘛。
