...
cc程式/                      ← 專案根目錄（每台機器各自 clone，不放 OneDrive）
  ...
  docs/                      ← 跨子專案共用文件
    ...
    機器設定指南.md            ← 新機器設定步驟
    methodology-self-qa.md    ← 方法論：自問自答法
```
...
遇到「多個合理方案需取捨」的設計問題時，先自問自答迭代收斂，再提交用戶審閱。

- **適用**：架構設計、發現缺口需補丁、用戶授權自行思考
- **不適用**：執行類任務、用戶已給具體指令、簡單 bug 修復
- **判斷**：有唯一正確答案 → 直接做。有多個合理方案 → 自問自答
- **品質**：必須有收斂（越問越具體）、有自我否定（至少砍掉一樣東西）、有回頭驗算
- **詳見**：`docs/methodology-self-qa.md`
