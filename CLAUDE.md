...
| Topic ID 重複 | `/更新` 時掃描偵測，提醒用戶合併 |
| CLAUDE.md 並行編輯 | git auto-merge。`/更新` 時讀 diff 檢查語義一致性，矛盾時提醒用戶裁決 |
| 程式碼衝突 | 讀懂兩邊改動，合併成正確版本，跑測試驗證 |
| package-lock.json | 刪掉 `package-lock.json`，重新 `npm install` |
| 暫存檔矛盾（相反結論） | 兩份都保留，`/修改計畫` 時列出讓用戶裁決 |
| 平行決策（同 topic 多台） | 啟動流程自動偵測（同 topic 出現在多台機器 → 標出提醒用戶） |
