...
**全能標案助理（標案自動駕駛）**——機器接管執行，Jin 在關鍵節點做決策。飛輪三齒輪：選案 → 寫作 → 學習。最高導航：`docs/product-compass-v0.6.md`

| 組件 | 路徑 | 狀態 | 說明 |
|------|------|------|------|
| 主程式 Web App | `bidding-assistant/` | 開發中 | Next.js 16 + React 19，16 個功能模組（3295 tests） |
...
| # | 階段 | 核心動作 | 已完成模組 | 開發狀態 |
|---|------|---------|----------|---------|
| 1 | 找案 | 自動巡標、初步分類、推送符合條件的案件 | W01 自動化完成（2921 tests） | ✅ 完成 |
| 2 | 情報蒐集 | 自動彙整情報摘要，標註關鍵風險與機會 | M01 + PCC 情報模組 + Explorer 可鑽探搜尋 | ✅ 完成 |
| 3 | 外部資源整合 | 從知識庫推薦適配人選，附歷史合作紀錄 | 00A 團隊成員資料庫（計畫中） | ⏳ 待開發 |
| 4 | 評估篩選 | 自動產出評估報告（勝率預估、資源需求、風險 flag） | M03 戰略分析 + M04 品質閘門（82+114 tests） | ✅ 完成 |
| 5 | 建議書撰寫 | 三階段自動化（勾勒方向 → 框架展開 → 排版整合） | M02 知識庫 v0.1 規格完成；視覺生成方案待定 | ⏳ P1 開發 |
| 6 | 報價分析 | 自動產出報價試算表與利潤分析 | 報價模組重構（28 tests） | ✅ 完成 |
| 7 | 資格文件 | 自動盤點缺件、產出檢查清單 | 品質檢查模組重構（114 tests） | ✅ 完成 |
| 8 | 送件 | 格式檢查、完整性驗證、產出送件包 | M06 排版輸出模組完成（96 tests） | ✅ 完成 |
| 9 | 評選簡報 | 自動產出簡報初版，附建議講稿 | M06 排版輸出的 PPT 路徑 | ✅ 完成 |
| 10 | 議價 | 提供議價區間建議與底線分析 | 成本分析邏輯（M03+M06 涵蓋） | ⏳ 待補強 |
| 11 | 履約管理 | 自動產出工作計畫書、進度排程、報告文件 | 提案組裝引擎（31 tests） | ⏳ 待延伸 |
| 12 | 結案 | 自動產出結案文件，關鍵資料回流知識庫 | 00E 案後檢討筆記（KB 初始化時啟動） | ⏳ 待開發 |

## 6. 三階段遷移路徑

### 現況：第一階段「本地開發」

**目標**：確保程式碼能順利過渡到上雲環境，建立可測試的自動化管線。

| 檢查點 | 狀態 | 說明 |
|--------|------|------|
| 無硬編碼本地路徑 | ✅ | 已納入部署約束檢查清單（bidding-assistant/CLAUDE.md） |
| 跨平台相容性 | ✅ | 215 檔 3295 tests 已驗證 |
| API 資料存取 | ✅ | PCC/SmugMug MCP + Notion API 完成；Supabase 結構決策已定 |
| 環境變數管理 | ⏳ | `.env.example` 範本待完善 |

### 預計：第二階段「上線部署」

**條件**：知識庫初始化完成、三件最小展示版驗收通過

- Supabase 多租戶認證架構部署（Layer 0 基礎建設）
- Web App + SaaS 雙軌部署（分套件管理）
- CI/CD 自動化（GitHub Actions）
- 資料備份 / 恢復機制（四層沙盒確認）

### 遠期：第三階段「正式營運」

**里程碑**：用戶實際操作、反饋回流、持續迭代

- 負載測試 & 性能優化
- 多人協作功能（即時 sync）
- 知識庫飛輪自動化（結案 → 回流 → 下一案優化）
