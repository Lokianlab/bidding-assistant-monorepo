...
rebase-standard|進行中|P2|git pull --rebase 標準化|Z1FV|JDNE 同意（建議 fetch+rebase），待其他機器表態|0223
verification-queue|進行中|P2|驗收佇列機制|Z1FV|JDNE 同意（建議放 /待辦），待其他機器表態|0223
optimize-add-cut-add|進行中|P2|「先加再砍再加回」優化方法論|3O5L|A44T+JDNE+ITEJ 同意。各補：品質門檻/展示要求/scope guard/觸發條件/砍一次一項/展示有硬性停點和推理鏈兩種觸發。待 AINL/Z1FV 表態|0223
