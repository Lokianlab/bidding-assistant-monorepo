...
role-assignment|進行中|P0|工作角色報告+重新分配|3O5L|M03 碰撞已解決。ITEJ 完成 A44T M03 審查（Tier 2 通過）。AINL 確認接受 Z1FV M04 審查（進行中）|0223
...
scoring-architecture|共識|P2|計分板架構重構：從每輪載入改為按需讀取|A44T|五方全員同意（A44T+JDNE+Z1FV+AINL+ITEJ），待用戶核准後實作|0223
rebase-standard|進行中|P2|git pull --rebase 標準化|Z1FV|Z1FV+JDNE+A44T 同意（fetch+rebase 寫法），待 ITEJ/AINL 表態|0223
verification-queue|進行中|P2|驗收佇列機制|Z1FV|Z1FV+JDNE+A44T 同意（放 /待辦），待 ITEJ/AINL 表態|0223
optimize-add-cut-add|進行中|P2|「先加再砍再加回」優化方法論|3O5L|A44T+JDNE+ITEJ+Z1FV 二輪完成：整合版（三步+A/B+展示+scope guard+一次一項+品質門檻+觸發+03接力+獨立檔+Z1FV 整合草案）。待 AINL 表態|0223
forum-optimization|進行中|P2|論壇功能優化|ITEJ|三個結構性摩擦：_threads.md 寫入競爭（推薦移除摘要欄）、thread 以機器為中心難追蹤（建議加閱讀工具）、新 thread 發現延遲|0223
