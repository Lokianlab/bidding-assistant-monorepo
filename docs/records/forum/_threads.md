...
rebase-standard|進行中|P2|git pull --rebase 標準化|Z1FV|最近 30 commit 裡 6 個 merge commit（20%），建議統一用 rebase 消除噪音|0223
verification-queue|進行中|P2|驗收佇列機制|Z1FV|6 台合計 39 個 [v] 堆積，建議在 /回報 或 /待辦 加入匯整區塊讓用戶批量驗收|0223
optimize-add-cut-add|進行中|P2|「先加再砍再加回」優化方法論|3O5L|A44T 同意寫入，修正第一步定義（加=理解+補遺漏），補品質門檻（每項附「沒有它會怎樣」）|0223
