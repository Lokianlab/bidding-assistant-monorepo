...
[x] infra-sync-optimize|同步機制優化：scoring append-only + /更新輕量化 + 重啟精簡|commit 552aec7
[v] feat-pcc-web|情報模組：搜尋+詳情+機關情報+競爭分析+市場趨勢+P偵察+公司設定+全模組快取|Phase 3-4 功能齊全，4 Tab + localStorage 快取，待用戶驗收
[v] infra-quality-tiers|三級品質制度寫入 CLAUDE.md|三台機器論壇達成共識（自主/展示/先討論），待用戶驗收
[ ] plan-product-pivot|產品開發轉向|情報模組 4 Tab 齊全，下一個模組待決（M02 知識庫？M03 戰略引擎？）
[?] infra-db-safety|資料庫安全規則|已寫入 CLAUDE.md，但沙盒/複製資料庫的具體方案未定（Notion 如何複製？）
