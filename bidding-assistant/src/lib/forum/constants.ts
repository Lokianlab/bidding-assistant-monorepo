...
export const APPROVAL_SUMMARIES: Record<string, { what: string; detail: string; approve: string; reject: string }> = {
  ...
  "scoring-architecture": {
    ...
    detail: "問題：計分板 15KB，每次新 session 載入全部，其中歷史記錄佔 80% 但只在查詢時才需要。方案：拆三檔——規則（常駐，3KB）、歷史（按需讀取）、快照提醒（每次載入的簡要摘要）。效益：每次啟動省 12KB token。風險：拆檔後歷史查詢多一步。Jin 的退回意見：如何評估方案是否有效？需要說明。",
    approve: "開始拆檔案",
    reject: "維持每次載入全部記錄",
  },
  "rebase-standard": {
  ...
  "approval-tracking": {
    what: "批准後誰負責執行——跟你對話的那台機器就是執行方",
    detail: "問題：你批准一項決策後，不確定誰去執行。多台機器可能同時動手衝突，或都以為別人會做。方案：你批准時正在跟你對話的那台機器就是執行方；沒人在對話的情況下由 JDNE（機器主官）接。提案時建議標明「我來執行」。效益：每個批准都有明確的執行人。Jin 的退回意見：如果固定是同一個機器，斷線或被刪除的時候誰做？",
    approve: "確認這條分工規則",
    reject: "你想要不同的分工方式",
  },
};
