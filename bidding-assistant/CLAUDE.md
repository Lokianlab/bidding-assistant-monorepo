...
> 閉環有兩層，缺一不可：
>
> 1. **做對了**（正確性）— 修改本身無錯誤、無遺漏、全域一致。
> 2. **有效**（目的性）— 修改達成了它原本要解決的目的。
>
> 測試全過、grep 零殘留，是「做對了」。功能真的能用、問題真的被解決，才是「有效」。
> 兩層都通過，閉環才算完成。
...
8. **自證正確** — build 通過 + 測試通過（= 做對了）
9. **自證有效** — 回到原始需求，確認問題真的被解決（= 有效）

**禁止**：修改後未經驗證就交付。「做對了」和「有效」都必須驗證，兩者都通過才算閉環完成。
