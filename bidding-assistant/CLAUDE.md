...
> 閉環有四層，有順序，缺一不可。第一層沒過就不用談後面的。
>
> 1. **做對了**（正確性）— 修改本身無錯誤、無遺漏。
> 2. **有效**（目的性）— 修改達成了它原本要解決的目的。
> 3. **做全了**（完備性）— 邊界條件和錯誤路徑都有處理（或有不處理的理由）。
> 4. **沒矛盾**（一致性）— 改了程式碼，文件和設定也跟著對上了。
>
> 完整四層驗證說明見 `docs/methodology/closed-loop.md`。
...
| 文件 / 設定檔 | 全域搜尋（grep 舊值） | 殘留為零、所有副本一致 |
| 環境變數 / 欄位改名 | grep 全 codebase 所有引用處 | 無斷鏈、無不一致 |
| 刪除功能 / 移除依賴 | grep 確認無殘留引用 | 零殘留 |

#### 第二層：有效（驗證目的性）

做對了不等於有效。必須回頭確認：**這個修改原本要解決什麼問題？那個問題現在真的解決了嗎？**

- 改了環境變數名稱 → 程式能實際連上服務嗎？
- 修了一個 bug → 原本的異常行為消失了嗎？
- 加了新功能 → dev server 上實際操作一遍，行為符合預期嗎？
- 改了文件 → 照著文件走一遍，步驟正確嗎？

無法當下驗證時（例如需要外部服務、需要用戶操作），必須明確標註「待驗證」及驗證方式，不可默認有效。

#### 第三層：做全了（驗證完備性）

主流程通了不代表所有情況都通了。四個面向逐一確認：
...
優先檢查最可能發生的和後果最嚴重的。每個邊界都有明確決定——「有處理」或「不處理，理由是 XX」。
...
|---------|-----------|
| 程式碼 ↔ 文件 | 實際行為跟文件描述一致嗎？ |
| 設定檔 ↔ 實際行為 | 設定值跟系統實際使用的值一致嗎？ |
| 模組 ↔ 模組 | 改了模組 A，依賴 A 的模組假設還成立嗎？ |
...
8. **自證正確** — build 通過 + 測試通過（= 第一層）
9. **自證有效** — 回到原始需求，確認問題真的被解決（= 第二層）
10. **自證完備** — 邊界條件有處理或有不處理的理由（= 第三層）
11. **自證一致** — 相關文件、設定、依賴模組都更新了（= 第四層）

**禁止**：修改後未經驗證就交付。四層都通過才算閉環完成。
