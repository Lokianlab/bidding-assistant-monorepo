...
閉環步驟：寫代碼 → 自我審查 → 寫測試 → `npm test` → `npm run build` → 確認問題真的被解決 → 邊界條件有處理或理由 → 文件/設定一致。

**禁止**：修改後未經驗證就交付。詳細驗證表見 `docs/methodology/closed-loop.md`。
...
根據根目錄 CLAUDE.md 的部署約束，新代碼必須符合以下標準：

| 項目 | 檢查方式 | 通過標準 |
|------|--------|---------|
| 無硬編碼路徑 | `grep -r "C:\\\|./\|~/" src/` | 無結果或全是註解 |
| 跨平台相容 | macOS 上跑 `npm test && npm run build` | 全部過 |
| 環境變數機密 | `git diff HEAD~1 \| grep -i 'key\|secret\|token'` | 無結果 |
| 資料庫隔離 | 確認讀/寫路徑（若涉及 Supabase）經 API 層 | 無直接資料庫連線 |

不符合任何一項 → **無法提交**，由開發者和審查者負責修正。（出處：根目錄 CLAUDE.md 部署約束）
